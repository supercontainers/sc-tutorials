...
Now, start the container interactively and look at the libraries used for *osu_latency* and the environment. Is this what you expect?
...
Rebuild with this, remembering to iterate your version number once again, and then run interactively to understand what has happened. 
...
Another important thing to note is that libraries you bring in from the host might require other libraries that aren't required in the version within the container. For example, *libmpi* might require *libfabric* which might require *libcxi* at some HPC centers. Our base containers may be fairly portable, but it often is quite a bit of work on an individual system to make your application performant. For this reason, some HPC centers use wrappers to provide the host system functionality for containers. For example, podman-hpc has a *--cuda-mpi* flag at NERSC that will insert the CUDA aware vendor tuned MPICH on Perlmutter. Options like this might be available for you, and likely work well for most containerized applications.  
...
### EXAMPLE: Run a molecular dynamics simulation on a GPU with containers
