...
keypoints:
- Download a container image with `singularity pull` 
- Execute commands in a container with `singularity exec`
- By default Singularity pass all host variables to the container
- By default Singularity use the host current directory as the container working directory
...
```bash
cd
git clone https://github.com/supercontainers/sc-tutorials
...
Download Ubuntu 22.04 image using:
...
```bash
singularity exec ubuntu_22.04.sif whoami
```
```output
trainingxx
```
...
```bash
singularity exec -e ubuntu_22.04.sif bash -c 'echo $HELLO'
```
```output
...
singularity exec -e ubuntu_22.04.sif bash -c 'echo $BYE'
```
```output
moon
```
...
```bash
singularity exec -e --env BYE="moon" ubuntu_22.04.sif bash -c 'echo $BYE'
```
```output
moon
```
...
```bash
export HELLO="world"
podman run ubuntu:22.04 bash -c 'echo $HELLO'
```
```output
...
```bash
podman run -e HELLO ubuntu:22.04 bash -c 'echo $HELLO'
```
```output
world
```
...
```bash
podman run -e HELLO=moon ubuntu:22.04 bash -c 'echo $HELLO'
```
```output
moon
```
...
```bash
singularity exec ubuntu_22.04.sif ls /NODUS
```
```output
...
```bash
singularity exec -B /NODUS ubuntu_22.04.sif ls /NODUS
```
```output
...
### Do It Yourself (as time allows): BLAST example
